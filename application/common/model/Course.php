    ...
    public static  function  getAll($courseName=null){
        try {
            if (!$courseName) {
                ...
                $res = self::alias('C')->where(['C.siDel' => 0])->where('C.courseName','like ','%' . $courseName . '%')->leftJoin('CourseType T', 'C.courseType=T.id')->field('C.id,C.courseName,T.typeName,C.isShow,C.startTime,C.endTime,C.mobile,C.click,C.sort')->select();
            }
            $data['data'] = $res;
