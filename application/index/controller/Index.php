...
class Index extends Controller
{
    ...
    public function activity()
    {
        ...
        if(request()->isAjax()) {
            ...
            $all_activity = Db::connect('db_config1')->name('fx_activity')->field('id,title,thumb,intro,starttime,endtime,joinstime,joinetime')->where(['show'=>1,'merchantid'=>19])->order('displayorder desc')->limit(($p['current_page']-1)*$p['per_page'],$p['per_page'])->select();
            $total=Db::connect('
            db_config1')->where(['show'=>1,'merchantid'=>19])->name('fx_activity')->count();
